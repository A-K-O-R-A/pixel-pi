...
const BLOCK_SIZE: usize = 1024 * 1024; //1 mb = 1 Block
...
pub fn find_sequence(seq: &[u8], f: &mut File) -> io::Result<usize> {
    ...
    while (cur_offset as u64) <= file_size {
        ...
        if let Some(found_index) = cur_buffer[0..(n + seq_size)]
            .windows(seq_size)
            .position(|w| w == seq)
        {
        ...
        cur_offset += BLOCK_SIZE - seq_size;

        print!(
            "\rsearching... {:.2}%",
            (cur_offset as f64) * 100.0 / (file_size as f64)
        );
    }
...
/// Returns the offset at which point the provided sequence begins in the file (does not accountt for previous seeking)
#[allow(dead_code)]
pub fn find_sequence_lossy(seq: &[u8], f: &mut File) -> io::Result<usize> {
    let seq_size = seq.len();
    let file_size = f.metadata()?.len();
